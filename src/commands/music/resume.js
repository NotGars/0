...
export async function execute(interaction, context) {
  ...
  if (owner && !hasPerm) {
    return interaction.reply({ content: 'No tienes permiso para reanudar en este canal.', ephemeral: true });
  }
  if (!queue.paused) return interaction.reply({ content: 'La cola no está pausada.', ephemeral: true });
